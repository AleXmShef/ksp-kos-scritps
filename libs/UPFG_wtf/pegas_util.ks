...
FUNCTION targetSetup {
	...
	LOCAL targetVelocity IS ___tgtV:MAG.
...
FUNCTION setupUPFG {
	...
	LOCAL tgoV IS vecYZ(mnvrnode:deltav).
...
FUNCTION acquireStateDummy {
	//	Expects a global variable "liftoffTime" as scalar
	declare parameter dummyPos.
	declare parameter dummyVel.

	RETURN LEXICON(
		"time", TIME:SECONDS - liftoffTime:SECONDS,
		"mass", SHIP:MASS*1000,
		"radius", vecYZ(dummyPos) * -1,
		"velocity", vecYZ(dummyVel)
	).
}
...
FUNCTION userEventHandler {
	...
	ELSE IF eType = "warp" OR eType = "w" {
		set kuniverse:timewarp:mode to "PHYSICS".
		set kuniverse:timewarp:warp to 2.
		local i is userEventPointer.
		WHEN (TIME:SECONDS >= (liftoffTime:SECONDS + sequence[i]["time"] + sequence[i]["dur"])) THEN {
			set kuniverse:timewarp:mode to "PHYSICS".
...
GLOBAL parabolicPitchGuidanceParams IS LEXICON (
	"A", 0
).

FUNCTION parabolicPitchGuidance {
	DECLARE PARAMETER mode TO 0. //  0 - main run, 1 - initialization

	IF (mode = 1) {				 //  Calculate parabolic formula
		SET terminatingPitchRad TO TAN(controls["terminatingPitch"]).
		SET _mTime TO controls["upfgActivation"] - controls["verticalAscentTime"].
		SET parabolicPitchGuidanceParams["A"] TO SQRT(_mTime) * 2 * terminatingPitchRad.
	}
	ELSE IF (mode = 0) {
		SET _mMissionTime to (TIME:SECONDS - controls["verticalAscentTime"]) - liftoffTime:SECONDS.
		SET _mMissionTime to _mMissionTime * (_mMissionTime/(controls["upfgActivation"] - controls["verticalAscentTime"])).
		SET targetPitchTan TO (parabolicPitchGuidanceParams["A"]*SQRT(_mMissionTime))/(2*_mMissionTime).
		SET targetPitchRad TO ARCTAN(targetPitchTan).
		RETURN targetPitchRad.
	}
}
