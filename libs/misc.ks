...
declare function CWequationCorrectionBurn {
	declare parameter RbarChaserPosition.
	declare parameter arrivalTime.
	declare parameter vessel.
	declare parameter precisionMode to 1.

	local dVtreshold is 0.05.
	local tickrate is 0.1.
	local ipu is 1000.

	if(precisionMode = 2) {
		set dVtreshold to 0.006.
		set tickrate to 0.
		set ipu to 2000.
	}
	...
	SET TargetOrbit TO UpdateOrbitParams(TargetOrbit, vessel:ORBIT).

	LOCAL RbarTargetPosition IS RatAngle(TargetOrbit, AngleAtT(TargetOrbit, vessel:ORBIT:TRUEANOMALY, arrivalTime - TIME:SECONDS)).
	local prevConf is config:ipu.
	...
	until(done = 1) {
		...
		LOCAL departureChaserPosition IS RatAngle(CurrentOrbit, SHIP:ORBIT:TRUEANOMALY).
		LOCAL departureTargetPosition IS RatAngle(TargetOrbit,vessel:ORBIT:TRUEANOMALY).

		LOCAL departureChaserVelocity IS VatAngle(CurrentOrbit, SHIP:ORBIT:TRUEANOMALY).
		LOCAL departureTargetVelocity IS VatAngle(TargetOrbit, vessel:ORBIT:TRUEANOMALY).

		LOCAL departureChaserTargetVelocity IS CWgetVelocityFromPositions(
			departureChaserPosition,
			RbarChaserPosition,
			arrivalTime - time:seconds,
			TargetOrbit,
			departureTargetPosition,
			departureTargetVelocity,
			RbarTargetPosition,
			departureChaserVelocity).

		LOCAL dV IS nodeFromV(CurrentOrbit, departureChaserPosition, departureChaserVelocity, departureChaserTargetVelocity)["dVvec"].
		if(_dV = -1)
			set _dV to dV.
		PRINT ("Dv mag:" + dV:mag) at (0, 7).

		IF(dV:MAG < dVtreshold) {
		...
		IF(precisionMode = 2 AND time:seconds  - _dV:MAG/(0.2/5) > ctime) {
			SET SHIP:CONTROL:TRANSLATION TO V(0, 0, 0).
			SET done TO 1.
		}
		wait tickrate.
	}
	set config:ipu to prevConf.
}
